...
#
# Check services.pl (standard) vars
#
# (I):
...
_toolsdir="$(toolsdir)"
color_blue "[${thisbase}] checking services.pl vars"
...
_countout=$(cat "${_fout}" | grep -v WAR | wc -l)
_res="$(grep -v WAR "${_fout}" | awk '{ print $2 }')"
if [ ${_rc} -eq 0 -a ${_counterr} -eq 0 -a ${_countout} -eq 1 -a "${_res}" = "service_site_value" ]; then
    echo "${_res} - OK"
    (( _count_ok += 1 ))
else
...
_countout=$(cat "${_fout}" | grep -v WAR | wc -l)
_res="$(grep -v WAR "${_fout}" | awk '{ print $2 }')"
if [ ${_rc} -eq 0 -a ${_counterr} -eq 0 -a ${_countout} -eq 1 -a "${_res}" = "service_node_value" ]; then
    echo "${_res} - OK"
    (( _count_ok += 1 ))
else
...
_countout=$(cat "${_fout}" | grep -v WAR | wc -l)
_res="$(grep -v WAR "${_fout}" | awk '{ print $2 }')"
if [ ${_rc} -eq 0 -a ${_counterr} -eq 0 -a ${_countout} -eq 1 -a "${_res}" = "node_service_value" ]; then
    echo "${_res} - OK"
    (( _count_ok += 1 ))
else
