...
sub new
{
    ...
    my $configuration = retrieveConfiguration( \%params );
...
# patched to provide the path to credentials.ini

sub retrieveConfiguration
{
	my( $params ) = @_;
	my $fname = "";
	$fname = $params->{credentials} if exists $params->{credentials};
	
	my @candidates = ();
	push( @candidates, $fname ) if $fname;
	push( @candidates, "$ENV{PWD}/ovh.conf", "$ENV{HOME}/.ovh.conf", '/etc/ovh.conf' ) if $Config{osname} ne 'MSWin32';

    my $fh;
    foreach my $filepath ( @candidates )
    {
...
sub rawCall
{
    ...
    if (not $params{'path'})
    {
        ...
        return TTP::OvhAnswer::->new(response => HTTP::Response->new( 500, "Missing parameter: path", [], '{"message":"Missing parameter: path"}'));
    }
    if (not $params{'method'})
    {
        carp "Missing parameter: method";
        return TTP::OvhAnswer::->new(response => HTTP::Response->new( 500, "Missing parameter: method", [], '{"message":"Missing parameter: method"}'));
    }
    ...
    unless ($params{'noSignature'})
    {
        ...
        if (not $self->{'consumerKey'})
        {
            ...
            return TTP::OvhAnswer::->new(response => HTTP::Response->new( 500, "Performed an authentified call without providing a valid consumerKey", [], '{"message":"Performed an authentified call without providing a valid consumerKey"}'));
        }
    ...
    return TTP::OvhAnswer::->new(response => $UserAgent->$method($url, %httpHeaders, %content));
}
...
Answer are retured as instances of L<TTP::OvhAnswer|TTP::OvhAnswer>.
